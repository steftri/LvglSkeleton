...
WiFiClient m_MqttTcpClient;
MqttClient myMqttClient(&m_MqttTcpClient); 
...
void MqttHal::setup()
{
  ...
  myMqttClient.setId("ESP32Client"); // Set the MQTT client ID
  myMqttClient.onMessage(MqttHal::onMessage); // Set the callback for incoming messages
}
...
MqttHal::ERc MqttHal::connect(const char *pc_BrokerAddress, uint16_t u16_BrokerPort)
{
  ...
  if (myMqttClient.connect(pc_BrokerAddress, u16_BrokerPort))
  {
    m_actionListener.onConnected();
  }
  else
  {
    error = myMqttClient.connectError();
...
void MqttHal::setLastWill(const char *pc_Topic, const uint8_t *pu8_MessageBuffer, const size_t MessageSize, uint8_t u8_QoS, bool b_Retain)
{
  ...
  myMqttClient.beginWill(pc_Topic, u8_QoS, b_Retain);
  myMqttClient.write(pu8_MessageBuffer, MessageSize);
  myMqttClient.endWill();
}
...
void MqttHal::publish(const char *pc_Topic, const uint8_t *pu8_MessageBuffer, const size_t MessageSize, uint8_t u8_QoS, bool b_Retain)
{
  ...
  myMqttClient.beginMessage(pc_Topic, MessageSize, u8_QoS, b_Retain);
  myMqttClient.write(pu8_MessageBuffer, MessageSize);
...
void MqttHal::onMessage(int messageSize)
{
  ...
  String topic = myMqttClient.messageTopic();
  String message = myMqttClient.readString();
